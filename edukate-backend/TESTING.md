...
- **Integration tests**: `@SpringBootTest` + `WebTestClient`. Use Testcontainers for MongoDB and MinIO. Use an embedded/mock RabbitMQ.
...
Integration tests use `@SpringBootTest(webEnvironment = RANDOM_PORT)`, Testcontainers MongoDB, a mock `FileKeyStorage`, and `NoopNotifier`.
