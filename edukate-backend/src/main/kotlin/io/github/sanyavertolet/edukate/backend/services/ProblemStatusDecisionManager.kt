...
@Service
class ProblemStatusDecisionManager(private val userProblemStatusRepository: UserProblemStatusRepository) {
    fun getStatus(userId: String, problemId: String): Mono<Problem.Status> =
    ...
    fun getStatus(problemId: String, authentication: Authentication?): Mono<Problem.Status> =
        authentication.monoId().flatMap { userId -> getStatus(userId, problemId) }.defaultIfEmpty(Problem.Status.NOT_SOLVED)

    /**
     * bestStatus == SubmissionStatus.SUCCESS => Problem.Status.SOLVED bestStatus == SubmissionStatus.FAILED =>
     * Problem.Status.FAILED bestStatus == SubmissionStatus.PENDING => Problem.Status.SOLVING else =>
