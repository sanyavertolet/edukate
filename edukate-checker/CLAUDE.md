...
AI-powered problem checking service. Receives check requests from the backend via RabbitMQ, fetches submission images from S3, calls OpenAI via Spring AI, and publishes results back.
...
- OpenAI: system prompt (`OPENAI_SYSTEM_PROMPT`), model (`OPENAI_MODEL`), API key (`OPENAI_API_KEY`), temperature fixed at 1.0
...
All Java files need to be converted to Kotlin. The two Kotlin data classes (`RequestContext`, `ModelResponse`) stay as-is.
...
- Remove all Lombok annotations (`@RequiredArgsConstructor`, `@Slf4j`, `@Value`): use constructor injection and `private val log = LoggerFactory.getLogger(...)` or `KotlinLogging.logger {}`.
