...
class RequestContextTest {
    ...
    fun `valid context is created`() {
        assertThatCode { RequestContext("Solve x^2 = 4", media, media) }.doesNotThrowAnyException()
    }

    @Test
    fun `blank problem text throws`() {
        assertThatThrownBy { RequestContext("", media, media) }.isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun `empty submission images throws`() {
        assertThatThrownBy { RequestContext("Solve x^2 = 4", media, emptyList()) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun `empty problem images is allowed`() {
        assertThatCode { RequestContext("Solve x^2 = 4", emptyList(), media) }.doesNotThrowAnyException()
    }
}
