...
> - `get*ResponseMock(override?)` — typed faker data matching the OpenAPI schema; accepts a partial override to pin fields
> - `get*MockHandler(override?)` — ready-made MSW handler; defaults to the faker mock, can be overridden per-test via `server.use(getGetProblemMockHandler({ name: "prob-1" }))`
