...
describe("NotificationButton — authenticated", () => {
    ...
    it("shows unread badge count when there are unread notifications", async () => {
        server.use(
            getGetNotificationsMockHandler(getGetNotificationsResponseMock({ statistics: { unread: 5, total: 10 } })),
        );
        render(<NotificationButton />);
        await waitFor(() => {
    ...
    it("opens the notification menu on button click", async () => {
        server.use(
            ...
            http.get("*/api/v1/notifications", () =>
                HttpResponse.json({ notifications: [], statistics: { total: 0, unread: 0 } }),
            ),
        );
        render(<NotificationButton />);
