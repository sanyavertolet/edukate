...
import { useProblemSetChangeUserRoleMutation, useProblemSetUserListQuery } from "@/features/problem-sets/api";
import { useAuthContext } from "@/features/auth/context";
import { InvitedUserManagement } from "./InvitedUserManagement.tsx";
...
const UserRoleManager: FC<UserRoleManagerProps> = ({ shareCode }) => {
    const changeUserRoleMutation = useProblemSetChangeUserRoleMutation();
    const { data: userNameWithRoleList, refetch: refetchUserList } = useProblemSetUserListQuery(shareCode);
    ...
    const userRole = useMemo(
        () => userNameWithRoleList?.find((userWithRole) => userWithRole.name == user?.name)?.role || "USER",
                        ...
                        secondaryAction={
                            <FormControl>
                                ...
                                >
                                    ...
                                    <MenuItem value={"MODERATOR"} disabled={userRole == "USER"}>
                                        Moderator
                                    </MenuItem>
...
export const ProblemSetUserManagement: FC<ProblemSetUserManagementProps> = ({ shareCode }) => {
    return (
        <Box sx={{ borderRadius: 1 }}>
            <InvitedUserManagement shareCode={shareCode} />
