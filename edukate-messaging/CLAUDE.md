...
`RabbitAutoConfiguration` — Spring Boot autoconfiguration; declares exchanges, queues, and bindings automatically when
...
- When testing listener classes in `edukate-notifier` or `edukate-checker`, inject messages directly into the listener
  method rather than going through RabbitMQ
